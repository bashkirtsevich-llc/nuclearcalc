...
function Count_Func(ex : boolean; R : TE; x,y,z,_n,Fi : extended; var F : extended) : boolean;
...
begin
  ...
  repeat
    ...
    if s[i]<>',' then
      begin
        if Parse_Line(s,A) then
          begin
            if Count_Func(false,A,0,0,0,0,0,temp) then
              begin
    ...
    if Parse_Line(Copy(s,1,i-1),A) then
      begin
        if not(Count_Func(false,A,0,0,0,0,0,points[n].x)) then
          begin
            Delete_Tree(A);
            exit;
          end
    ...
    if s[i]<>';' then
      begin
        if Parse_Line(s,A) then
          begin
            if Count_Func(false,A,0,0,0,0,0,points[n].y) then
              begin
                Delete_Tree(A);
                m:=n-1;
                break;
    ...
    if Parse_Line(Copy(s,1,i-1),A) then
      begin
        if not(Count_Func(false,A,0,0,0,0,0,points[n].y)) then
          begin
            Delete_Tree(A);
            exit;
          end
...
begin
  ...
  repeat
    ...
    if Parse_Line(Copy(s,1,i-1),A) then
      begin
        if not(Count_Func(false,A,0,0,0,0,0,F)) then
          begin
            Delete_Tree(A);
            exit;
          end
    ...
    if s[i]<>';' then
      begin
        if Parse_Line(s,A) then
          begin
            if Count_Func(false,A,0,0,0,0,0,F) then
              begin
                Delete_Tree(A);
                CheckTable:=true;
                exit;
...
begin
  ...
  if s ='FI' then
    begin
      New(R); R^.Oper:=O_FI; R^.Left:=nil; R^.Right:=nil;
      Parse_Line:=true; exit;
    end;
  if s='PI' then
    begin
            ...
            begin
              ...
              repeat
                ...
                if Parse_Line(Copy(s,1,i-1),A) and
                   Count_Func(false,A,0,0,0,0,0,F) then
                  Delete_Tree(A);
                O1^.Value:=F;
                s:=Copy(s,i+1,length(s)-i);
                New(O2);
...
begin
  ...
  case R^.Oper of
    ...
    O_FI    : if ex then begin F:=Fi; Count_Func:=true; end;
    OValue  : begin F:=R^.Value; Count_Func:=true; end;
    OPlus   : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and Count_Func(ex,R^.Right,x,y,z,_n,Fi,q2) then
                begin F:=q1+q2; Count_Func:=true; end
              else Count_Func:=false;
    OMinus  : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and Count_Func(ex,R^.Right,x,y,z,_n,Fi,q2) then
                begin F:=q1-q2; Count_Func:=true; end
              else Count_Func:=false;
    OMul    : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and Count_Func(ex,R^.Right,x,y,z,_n,Fi,q2) then
                begin F:=q1*q2; Count_Func:=true; end
              else Count_Func:=false;
    ODiv    : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and Count_Func(ex,R^.Right,x,y,z,_n,Fi,q2) and (q2<>0) then
                begin F:=q1/q2; Count_Func:=true; end
              else Count_Func:=false;
    OIntDiv : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and Count_Func(ex,R^.Right,x,y,z,_n,Fi,q2) and (q2<>0) then
                begin F:=Int(q1/q2); Count_Func:=true; end
              else Count_Func:=false;
    OPower  : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and Count_Func(ex,R^.Right,x,y,z,_n,Fi,q2) then
    ...
    ODx     : if Count_Func(ex,R^.Left,x+deltax,y,z,_n,Fi,q1) and
                 Count_Func(ex,R^.Left,x-deltax,y,z,_n,Fi,q2) then
                begin F:=(q1-q2)/(2*deltax); Count_Func:=true; end
              else Count_Func:=false;
    ODy     : if Count_Func(ex,R^.Left,x,y+deltax,z,_n,Fi,q1) and
                 Count_Func(ex,R^.Left,x,y-deltax,z,_n,Fi,q2) then
                begin F:=(q1-q2)/(2*deltax); Count_Func:=true; end
              else Count_Func:=false;
    OFx     : if Count_Func(ex,R^.Left,x+deltax,y,z,_n,Fi,q1) and
                 Count_Func(ex,R^.Left,x-deltax,y,z,_n,Fi,q2) then
                begin F:=((q2-q1)/(2*deltax)); Count_Func:=true; end
              else Count_Func:=false;
    OFy     : if Count_Func(ex,R^.Left,x,y+deltax,z,_n,Fi,q1) and
                 Count_Func(ex,R^.Left,x,y-deltax,z,_n,Fi,q2) then
                begin F:=((q2-q1)/(2*deltax)); Count_Func:=true; end
              else Count_Func:=false;
    OSin    : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) then
                begin F:=sin(q1); Count_Func:=true; end
              else Count_Func:=false;
    OCos    : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) then
                begin F:=cos(q1); Count_Func:=true; end
              else Count_Func:=false;
    OTan    : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and (cos(q1)<>0) then
                begin F:=sin(q1)/cos(q1); Count_Func:=true; end
              else Count_Func:=false;
    OCtg    : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and (sin(q1)<>0) then
                begin F:=cos(q1)/sin(q1); Count_Func:=true; end
              else Count_Func:=false;
    OSec    : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and (cos(q1)<>0) then
                begin F:=1/cos(q1); Count_Func:=true; end
              else Count_Func:=false;
    OCosec  : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and (sin(q1)<>0) then
                begin F:=1/sin(q1); Count_Func:=true; end
              else Count_Func:=false;
    OArcSin : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and (abs(q1)<=1.0) then
                if Abs(q1)=1.0 then
                  begin F:=q1*pi/2; Count_Func:=true; end
                else
                  begin F:=ArcTan(q1/sqrt(1-sqr(q1))); Count_Func:=true; end
              else Count_Func:=false;
    OArcCos : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and (abs(q1)<=1.0) then
                if q1=0 then
    ...
    OArcTan : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) then
                begin F:=ArcTan(q1); Count_Func:=true; end
              else Count_Func:=false;
    OArcCtg : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) then
                begin F:=pi/2-ArcTan(q1); Count_Func:=true; end
              else Count_Func:=false;
    OArcSec : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and (q1<>0) and (abs(1/q1)<=1.0) then
                if q1=0 then
                  begin F:=pi/2; Count_Func:=true; end
                else
                  begin
    ...
    OArcCosec : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and (q1<>0) and (abs(1/q1)<=1.0) then
    ...
    OSqrt   : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and (q1>=0) then
                begin F:=sqrt(q1); Count_Func:=true; end
               else Count_Func:=false;
    OSqr    : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) then
                begin F:=sqr(q1); Count_Func:=true; end
              else Count_Func:=false;
    OLn     : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and (q1>0) then
                begin F:=Ln(q1); Count_Func:=true; end
              else Count_Func:=false;
    OLg     : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and (q1>0) then
                begin F:=Ln(q1)/Ln(10); Count_Func:=true; end
              else Count_Func:=false;
    OAbs    : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) then
                begin F:=Abs(q1); Count_Func:=true; end
              else Count_Func:=false;
    OExp    : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) then
                begin F:=exp(q1); Count_Func:=true; end
              else Count_Func:=false;
    OSh     : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) then
                begin F:=(exp(q1)-exp(-q1))/2; Count_Func:=true; end
              else Count_Func:=false;
    OCh     : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) then
                begin F:=(exp(q1)+exp(-q1))/2; Count_Func:=true; end
              else Count_Func:=false;
    OTh     : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and ((exp(q1)+exp(-q1))<>0) then
                begin F:=(exp(q1)-exp(-q1))/(exp(q1)+exp(-q1)); Count_Func:=true; end
              else Count_Func:=false;
    OCth    : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and ((exp(q1)-exp(-q1))<>0) then
                begin F:=(exp(q1)+exp(-q1))/(exp(q1)-exp(-q1)); Count_Func:=true; end
              else Count_Func:=false;
    OSch    : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and ((exp(q1)+exp(-q1))<>0) then
                begin F:=2/(exp(q1)+exp(-q1)); Count_Func:=true; end
              else Count_Func:=false;
    OCsch   : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and ((exp(q1)-exp(-q1))<>0) then
                begin F:=2/(exp(q1)-exp(-q1)); Count_Func:=true; end
              else Count_Func:=false;
    OArSh   : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and ((q1+sqrt(sqr(q1)+1))>0) then
                begin F:=Ln(q1+sqrt(sqr(q1)+1)); Count_Func:=true; end
              else Count_Func:=false;
    OArCh   : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and (sqr(q1)>1) and((q1+sqrt(sqr(q1)-1))>0) then
                begin F:=Ln(q1+sqrt(sqr(q1)-1)); Count_Func:=true; end
              else Count_Func:=false;
    OArTh   : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and ((1-q1)<>0) and ((1-q1)*(1+q1)>0) then
                begin F:=ln(sqrt((1+q1)/(1-q1))); Count_Func:=true; end
              else Count_Func:=false;
    OArCth  : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and ((q1-1)<>0) and ((q1+1)/(q1-1)>0) then
                begin F:=ln((q1+1)/(q1-1))/2; Count_Func:=true; end
              else Count_Func:=false;
    OArSch  : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and (q1<>0) and (sqr(1/q1)>1) and((1/q1+sqrt(sqr(1/q1)-1))>0) then
                begin
                  F:=Ln(1/q1+sqrt(sqr(1/q1)-1));
                  Count_Func:=true;
                end
              else Count_Func:=false;
    OArCsch : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) and (q1<>0) and ((1/q1+sqrt(sqr(1/q1)+1))>0) then
                begin
                  F:=Ln(1/q1+sqrt(sqr(1/q1)+1));
                  Count_Func:=true;
                end
              else Count_Func:=false;
    ORound  : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) then
                begin F:=Round(q1); Count_Func:=true; end
              else Count_Func:=false;
    ORandom : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) then
                begin F:=Random(Round(q1)); Count_Func:=true; end
              else Count_Func:=false;
    OInt    : if Count_Func(ex,R^.Left,x,y,z,_n,Fi,q1) then
...
begin
  if Parse_Line(s, A) then
    if Count_Func(false, A, 0, 0, 0, 0, 0, r) then
      begin
        Delete_Tree(A);
        Count_Const:=true;
      end
