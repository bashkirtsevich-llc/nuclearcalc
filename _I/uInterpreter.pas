...
uses
  ...
  uFunctions, uCalcul, Forms;

type TDebuggerPos=procedure (Sender:TObject; Code: string; Pos: integer) of object;
...
type
  ...
  published
    ...
    property OnShowDebugPos:TDebuggerPos read fOnShowPos write fOnShowPos;
    ...
    property ErrorText:string read fErrText; {read only property}
    property ErrorPos:integer read fErrPos;
    procedure ContinueInterpreter;
    procedure HaltInterpreter;
    procedure Execute;
    procedure StepOver;
  end;
...
implementation
const cMaxWhile:integer=High(Integer); //max. while repeat count

function GetItemIndexByPos(text:string;Position:integer):integer;
var _temp:string;
    _pos:integer;
begin
  _temp:=text+#13#10;
  _pos:=0;
  Result:=0;
  while (_pos<>Position) do
  begin
    inc(_pos);
    if _temp[_pos]=#13 then Inc(Result);
  end;
end;
  ...
  while i<length(fprog) do begin
    ...
    if not (fProg[i] in [#0..#13,';']) then begin
      ...
      if (not z) and (not b) then begin
        ...
        if fProg[i] in ['v','V'] then begin
          t:=system.copy(fprog,i,length('VOID '));
          if Uppercase(t)='VOID ' then begin
            j:=i+length('VOID ');
...
procedure TInterpreter.ContinueInterpreter;
begin
  fDebug:=False;
  Pause:=False;
end;

procedure TInterpreter.StepOver;
begin
  Pause:=False;
  fCalcul.StepOver;
end;

procedure TInterpreter.HaltInterpreter;
begin
  fHalt:=True;
  fCalcul.Stop;
  Pause:=False;
...
begin
  ...
  _temp:=system.copy(fprog,ii,(i-ii));
  while (w<>'') and  (w[1] in ['{',' ',';']) do begin system.delete(w,1,1); end;
  if (w<>'') and (w<>';') then begin
    Count:=GetItemCount(w);
    //fCalcul.Formula:=w;
    try
      _length:=0;
      _pos:=Pos(w[1],_temp);
      ...
      begin
        fCalcul.Formula:=GetItemAtString(w,itemindex);
        if (fCalcul.Formula='')or(fCalcul.Formula=#13)or(fCalcul.Formula=#10) then continue;
        fCalcul.calc;
        Pause:=fDebug;
        if fDebug then
        begin
          //Show Interpreter position
          if Assigned(fOnShowPos) then fOnShowPos(Self,fCalcul.Formula,GetItemIndexByPos(fProg,(ii+_length+_pos+(2*itemindex))));
          while Pause do begin if fHalt then Exit; Application.HandleMessage; end;
        end;
        inc(_length,Length(fCalcul.Formula)+1);
        //fCalcul.Debug:=fDebug;
        if fCalcul.CalcError then begin
          fErr:=true;
          fErrText:=fCalcul.CalcErrorText+#13#10+' formula:'+fCalcul.Formula;
          ferrPos:=ii;
        end;
      end;
    except
...
begin
  ...
  if fCalcul=nil then begin
    fErr:=true;
    fErrText:='Error, CALCUL object is nil';
    exit;
  end;
  ...
  while i<length(fprog) do begin
    ...
    'r','R':
      begin //return
        t:=uppercase(system.copy(fProg,i,7));
        if (system.copy(t,1,6)='RETURN') and (length(t)>6) and (t[7] in ['{','}',' ',';',#0..#13]) then begin
          //debug stop
          go;
          if fDebug then
          begin
            if Assigned(fOnShowPos) then fOnShowPos(Self,t,GetItemIndexByPos(fProg,i+1));
            while Pause do begin if fHalt then Exit; Application.HandleMessage; end;
          end;
          //
    ...
    'i','I':
      begin //IF
        ...
        if t='IF ' then begin
          ...
          if fDebug then
          begin
            if Assigned(fOnShowPos) then fOnShowPos(Self,t,GetItemIndexByPos(fProg,i-3));
            while Pause do begin if fHalt then Exit; Application.HandleMessage; end;
          end;
          //
          go;
          ...
          if fDebug then
          begin
            if Assigned(fOnShowPos) then fOnShowPos(Self,t,GetItemIndexByPos(fProg,i-4));
            while Pause do begin if fHalt then Exit; Application.HandleMessage; end;
          end;
          //
    ...
    'e','E':
      begin //EXEC , EXIT ,ELSE, END.
        ...
        if (system.copy(t,1,4)='ELSE') and (length(t)>4) and (t[5] in [' ','{',#0..#13]) then begin
          ...
          if fDebug then
          begin
            if Assigned(fOnShowPos) then fOnShowPos(Self,t,GetItemIndexByPos(fProg,i-ii));
            while Pause do begin if fHalt then Exit; Application.HandleMessage; end;
          end;
          if fErr then begin ferrPos:=i;exit;end;
          i:=i+4;
    ...
    'w','W':
      begin //WHILE
        ...
        if t='WHILE ' then begin
          ...
          if fDebug then
          begin
            if Assigned(fOnShowPos) then fOnShowPos(Self,t,GetItemIndexByPos(fProg,i));
            while Pause do begin if fHalt then Exit; Application.HandleMessage; end;
          end;
          //
          if fErr then begin fErrPos:=i;exit;end;
      ...
      t:=uppercase(system.copy(fProg,i,9));
        if Copy(t,1,5)='CALL ' then begin
          ...
          if (t='') or (j<0) or (not (t[1] in ['A'..'Z','_'])) then begin
            fErr:=true;
            fErrText:='Invalid CALL procedure name from '+IntToStr(i);
            fErrPos:=i;
            exit;
          end;
